...
the number given as A literally. (This is also called an "immediate" value.)
If something says "register A", it means to use the number given as A to read
from (or write to) the register with that number. So, if the opcode addi adds
...
from common import ELFCODE_FUNCTIONS, Sample
...
if __name__ == "__main__":
    with open('input_1.txt') as samples:
        ...
        behaviours =\
            [[name for name, func in ELFCODE_FUNCTIONS.items() if
              sample.behaves_like(func)]
             for sample in samples]

        print(len([b for b in behaviours if len(b) >= 3]))
