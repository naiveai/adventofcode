...
import re
import math
from common import ELFCODE_FUNCTIONS, Instruction

if __name__ == "__main__":
    with open('input.txt') as instructions:
        instructions = list(map(lambda s: s.strip(), instructions.readlines()))

        instruction_pointer_index =\
            int(re.match(r"#ip *(\d)", instructions[0]).groups()[0])

        instructions = list(map(Instruction.from_str, instructions[1:]))

        regs = [1] + ([0] * 5)

        prev_pointer = -math.inf

        # The program goes through 1 cycle where it sets
        # up an absurdly large number in register 2,
        # then caclulates the sum of all the divisors of that
        # number, but it does it very badly. So we do it
        # instead, which takes a lot less time.

        while regs[instruction_pointer_index] < len(instructions):
            instruction_pointer = regs[instruction_pointer_index]

            if instruction_pointer < prev_pointer:
                sum_ = 0
                n = regs[2]
            ...
            regs =\
                ELFCODE_FUNCTIONS[instruction.opcode](regs,
                                                      *instruction.params)

            prev_pointer = instruction_pointer

            regs[instruction_pointer_index] += 1
